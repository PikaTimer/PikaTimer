...
public class ResultsDAO {
    ...
    private static final Map<String,Result> resultsMap = new ConcurrentHashMap<>(); 
    ...
    */
    ...
    public ObservableList<Result> getResults(Integer raceID) {
        ...
        if (raceResultsMap == null) {
            ...
            if(!raceResultsMap.containsKey(raceID)) raceResultsMap.put(raceID, FXCollections.observableArrayList());

            Task fetchResultsFromDB = new Task<Void>() {
                ...
                @Override public Void call() {
                   ...
                   System.out.println("ResultsDAO: Runing the getResults querry");
                    ...
                    results.stream().forEach(r -> {
                        ...
                        if(!raceResultsMap.containsKey(r.getRaceID())) raceResultsMap.put(r.getRaceID(), FXCollections.observableArrayList());

                        Platform.runLater(() -> {
                            raceResultsMap.get(r.getRaceID()).remove(r);
                            if(!r.isEmpty()){
                                raceResultsMap.get(r.getRaceID()).add(r);
                                System.out.println("ResultsDAO new result added " + r.getBib() + " from race " + r.getRaceID() + " new total " + raceResultsMap.get(r.getRaceID()).size() );
                            }
                        });
            ...
            new Thread(fetchResultsFromDB).start();
                ...
                @Override public Void call() {
                    ...
                    while(true) {
                        ...
                        try {
                            ...
                            resultsQueue.drainTo(pendingBibs,99);  // 100 total
                            
                            System.out.println("ProcessNewResult Thread: Processing: " + pendingBibs.size());

                            List<Result> pending = new ArrayList();
                            try {
                                pendingBibs.stream().forEach(s -> {
                                    List<Result> tmp=processBib(s);
                                    if (tmp != null) pending.addAll(tmp);
                                });
                            } catch (Exception e) {
                                e.printStackTrace();
                            } 
    ...
    public List<Result> processBib(String bib){
        ...
        waves.forEach(i -> {
            ...
            if (resultsMap.containsKey(bib + " " + r.getRaceID())) {
                r=resultsMap.get(bib + " " + r.getRaceID());
            }
